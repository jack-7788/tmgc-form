<template>
  <container-item-wrapper v-show="!widget.options.hidden" :widget="widget">
    ...
    >
      <el-table-column v-if="widget.options.showIndex" type="index" width="50" fixed="left" />
      <el-table-column
        v-if="widget.options.showCheckBox"
        type="selection"
          ...
          <template #default="scope">
            <template v-if="item.formatS === 'render' && !!item.render">
              <table-column-custom-render
                :row="scope.row"
                :column="item"
                :data-table-ref="getDataTableRef"
      ...
      <template v-if="!!widget.options.showButtonsColumn">
        ...
        >
          <template #default="scope">
            <template :key="inx" v-for="(ob, inx) in widget.options.operationButtons">
              <el-button
